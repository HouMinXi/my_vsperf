...
class Process(object):
    ...
    def _start_process(self):
        ...
        # rt kernel need qemu thread set the emulator priority
        # emulatorpin_qemu = subprocess.Popen(
        #     'sleep 1 && /usr/bin/pgrep -f qemu | /usr/bin/xargs -I {} /usr/bin/chrt -f -p 1 {}', shell=True,
        #     stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding="utf-8")
        # emulatorpin_qemu.wait(10)
        # if emulatorpin_qemu.poll() == 0:
        #     print(emulatorpin_qemu.communicate())
        # else:
        #     print("emulatorpin_qemu occur error returncode is %s" % emulatorpin_qemu.poll())
    ...
    def _affinitize_pid(self, core, pid):
        ...
        # use for rt kernel set thread priority to 1 and poll mode is fifo
        #run_task(['sudo', 'chrt' ,'-f', '-p', '1', str(pid)], self._logger)

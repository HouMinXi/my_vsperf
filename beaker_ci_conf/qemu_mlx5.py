...
class IVnfQemu(IVnf):
    ...
    def _configure_testpmd(self):
        ...
        self.execute_and_wait('rpm -ivh /root/dpdkrpms/1711-13/dpdk*.rpm')
        ...
        self.execute_and_wait('./tools/dpdk*bind.py --status')
        '''
        self.execute_and_wait('/usr/share/dpdk/usertools/dpdk-devbind.py --status')
        pci_list = ' '.join([nic['pci'] for nic in self._nics])
        #self.execute_and_wait('/usr/share/dpdk/usertools/dpdk-devbind.py -u ' + pci_list)
        #self._bind_dpdk_driver(S.getValue(
        #    'GUEST_DPDK_BIND_DRIVER')[self._number], pci_list)
        self.execute_and_wait('modprobe mlx5_core')
        pci_list1=pci_list.strip(' ').split(' ')
        self.execute_and_wait('/usr/bin/dpdk-devbind.py -b mlx5_core ' + pci_list1[0])
        self.execute_and_wait('/usr/bin/dpdk-devbind.py -b mlx5_core ' + pci_list1[1])
        self.execute_and_wait('ip a')
    ...
    def _bind_dpdk_driver(self, driver, pci_slots):
        ...
        if driver == 'uio_pci_generic':
            ...
            self.execute_and_wait('/usr/share/dpdk/usertools/dpdk-devbind.py -b uio_pci_generic '+
                                  pci_slots)
